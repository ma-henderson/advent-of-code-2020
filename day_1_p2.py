...
def threeto2020(values):
  # Attempt at "crawling method" but could not figure out smart algo way to adjust values when 'close'
    # also probably not a full proof method either.
  ...
  # Mix and Match between 2 bottom-tranche values and see if required 3rd val is IN list
  # Identify the upper limit of this operation (lowest val < 2020/2)
  for value in new_values:
    if value < 2020/2:
      break
    upper_lower = value
  # Check if 3-number combination is the solution
  # Build the lower list
  lower_list = []
...
def day_2(values):
  # POP LOCK AND DROP METHOD - BRUTE FORCE
  ...
  i_push = -1
  j_push = -1
  for i in new_values[:-3]:
    i_push += 1
    j_push = -1
    for j in new_values[1 + i_push:-2]:
      j_push += 1
      for k in new_values[2 + j_push:]:
        iterations.append((i,j,k))
        if i + j + k == 2020:
          print(i,j,k)
          exit
...
# Future improvements could be checks on each for loop if sum > 2020 (avoid a great majority of higher-end iterations)